...
public class PrimaryDirectionsServiceTest {
    static final Logger LOG = Logger.getLogger(com.theastrologist.service.PrimaryDirectionsServiceTest.class);
    ...
    @Autowired
    private PrimaryDirectionsService primaryDirectionsService;

    @Before
    public void setup() {
        samySkyPosition = themeService.getSkyPosition(TEST_DATE, LATITUDE, LONGITUDE);
        charlesFloquetSkyPosition = themeService.getSkyPosition(
                new DateTime(1828, 10, 2, 6, 30, CalcUtil.DATE_TIME_ZONE),
                new Degree(48, 51), new Degree(2, 17)
        );

        victorHugoSkyPosition = themeService.getSkyPosition(
                new DateTime(1802, 2, 26, 22, 30, CalcUtil.DATE_TIME_ZONE),
                new Degree(48, 51), new Degree(2, 21)
        );

        georgesClemenceauSkyPosition = themeService.getSkyPosition(
                new DateTime(1841, 9, 28, 21, 30, CalcUtil.DATE_TIME_ZONE),
                new Degree(46, 40), new Degree(0, 51)
        );

        esoMaliceSkyPosition = themeService.getSkyPosition(
    ...
    public void testPotentialPositions() {
        Set<PotentialDirection> potentialDirections = primaryDirectionsService.getPotentialDirections(samySkyPosition);
    ...
    private SortedSet<PrimaryDirection> getPrimaryDirections(SkyPosition skyPosition) {
        Set<PotentialDirection> potentialDirections = primaryDirectionsService.getPotentialDirections(skyPosition);
        return primaryDirectionsService.generatePotentialDirections(skyPosition, potentialDirections);
    }
